...
from traxis.graphics import tangent, JSONHandler
...
class MainWidget(skeleton.GuiSkeleton):
    ...
    def LoadFolder(self):
        """Connected to load Folder. Asks user to select folder using file browser. Passes selected directory
    ...
    def loadJSON(self, item):
        """ Loads a JSON file in the directory of choice when double clicked. Same as loadSession except fileName
            is determined by the saveSession double clicked.
        """
    ...
    def saveSession(self):
        ...
        dialog. Updates the Saved Sessions window using append_text() in JSONHandler.
        """
        ...
        print('filename_savesession', fileName)
        # return if no file was selected
        if not fileName:
            return
        ...
        y = fileName
        # Cuts off file name to get directory
        while y[-1] != '/':
            y = y[0:len(y) - 1]
        y = y[0:len(y) - 1]

        # Calls append_text() to update Saved Sessions window, if saved to the current folder
        if y == self.JSONTextBrowser.cd:
            self.JSONTextBrowser.append_text(y)
    ...
    def loadSession(self):
        ...
        # Calls append_text() to update Saved Sessions window
        self.JSONTextBrowser.append_text(y)
